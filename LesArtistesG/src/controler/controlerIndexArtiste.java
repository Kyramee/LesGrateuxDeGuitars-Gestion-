...
public class controlerIndexArtiste implements ActionListener {
		...
		if ( checkNumber( id ) ) {
			...
			if ( this.cs.getTabArtiste().isEmpty() ) {
				JOptionPane.showMessageDialog( parent,
						"Aucun ne artiste concorde avec vos param\u00EAtres de recherche", "Erreur",
	...
	private void ajouter( String nom, String membre, String url ) {
		if ( checkInfo( nom, url ) ) {
			if ( this.cs.containtArtiste( nom ) ) {
				...
				if ( JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog( parent,
						"Voulez-vous ajouter l'artiste " + nom + "?", "Confirmation ajout", JOptionPane.YES_NO_OPTION,
	...
	private Boolean checkInfo( String nom, String url ) {
		...
		ok = checkUrlVide( url, ok );

		return ok;
	}

	private Boolean checkUrlVide( String url, Boolean ok ) {
		if ( url.isEmpty() ) {
	...
	private Boolean checkNomVide( String nom, Boolean ok ) {
		if ( nom.isEmpty() ) {
